...
public class CommentController {
        ...
        return commentBaseService.generateComment(commentRequest)
                .thenApply(response -> {
                    ApiResponse<CommentResponse> apiResponse = response.isSuccess()
                            ...
                            : ApiResponse.error(response.getErrorMessage(), HttpStatus.INTERNAL_SERVER_ERROR);

                    return ResponseEntity.status(response.isSuccess() ? HttpStatus.OK : HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(apiResponse);
                })
                .exceptionally(e -> {
                    log.error("处理注释生成请求失败", e);
    ...
    public CompletableFuture<ResponseEntity<ApiResponse<List<CommentResponse>>>> batchGenerateComments(
        @Valid @RequestBody List<CommentRequest> commentRequests) {
        ...
        return commentExtendService.batchGenerateComments(commentRequests)
                .thenApply(response -> {
                    ...
                    ApiResponse<List<CommentResponse>> apiResponse = allSuccess
                            ...
                            : ApiResponse.error("部分注释生成失败", HttpStatus.INTERNAL_SERVER_ERROR);

                    return ResponseEntity.status(allSuccess ? HttpStatus.OK : HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(apiResponse);
                })
                .exceptionally(e -> {
                    log.error("批量生成注释失败", e);
                    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(ApiResponse.error(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR));
                });
    }
